...
Create a simple script that iterates over an array of fruit names and prints each one with its position
(e.g., "Fruit 1: apple").
First example is using the regular foreach-style loop, then refactored it to a C-style loop, and step-by-step explanations.
...
I was using a for loop in a previous shell script that involved installing packages.
So , here's what I was doing: `install_packages` function (which tracks progress),
add a counter to show the position (1-based, like "Fruit 1: apple"):
...
**Changes**:
- Add `count=0` to initialize a counter.
- Incremet `count` with `((count++))` inside the loop.
- Use `count` in the output to show the position.

This works but requires manually managing the `count` variable.

#### Refactored to C-Style Loop
Refactor the same script to use a C-style `for` loop, which uses an index to access array elements and eliminates the need for a separate counter:
...
To make the example slightly more interesting (and closer to the  `install_packages` function),  add a condition to simulate a “failure” for one fruit, similar to how you track failed packages:
...
- Used a string (`failed`) instead of an array for simplicity (to avoid Bash-specific arrays, though the  original script uses arrays).

### Applying to the `install_packages`
The `install_packages` function is more complex, but the refactoring follows the same principles:
...
- Keep arrays like `failed` since the script is Bash-specific.

The refactored `install_packages` from the previous response already applies this:
```bash
for ((i=0; i<${#pkgs[@]}; i++)); do
